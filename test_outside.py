...
def test_apply_modal_3():
    try:
        handle_dynamic_questions(driver)
        submit = driver.find_element(By.XPATH, "//button[.//span[text()='Next']]")
        submit.click()
    except Exception as e:
        logging.error(f"'test_apply_modal_3()': Test failed: {str(e)}")

try:
    with open('answer_cache.pkl', 'rb') as f:
        answer_cache = pickle.load(f)
except FileNotFoundError:
    answer_cache = {}

def save_cache():
    with open('answer_cache.pkl', 'wb') as f:
        pickle.dump(answer_cache, f)

def get_question_keyword(label_text):
    # Extract a keyword for the cache (customize as needed)
    return label_text.strip().lower()

def handle_dynamic_questions():
    # Find all label/input pairs in the modal
    labels = driver.find_elements(By.XPATH, "//label")
    for label in labels:
        label_text = label.text.strip()
        # Only process if label is visible and has text
        if not label_text:
            continue
        
        keyword = get_question_keyword(label_text)
        
        # Try to find the corresponding input
        try:
            input_elem = label.find_element(By.XPATH, "following-sibling::input")
        except Exception:
            continue 
        
        if keyword in answer_cache:
            answer = answer_cache[keyword]
            input_elem.clear()
            input_elem.send_keys(answer)
        else:
            # Prompt user for answer
            answer = input(f"Please answer for '{label_text}': ")
            answer_cache[keyword] = answer
            input_elem.clear()
            input_elem.send_keys(answer)
            save_cache()
